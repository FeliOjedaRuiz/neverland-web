...
- [x] **1.1** — `api/models/config.model.js`: Add `extrasCatalogo` subdocument array to Config schema
...
- [x] **1.2** — `api/models/event.model.js`: Add `catalogoItemIds` and `precioCatalogoApplied` to `detalles.extras`
...
- [x] **1.3** — `api/controllers/config.controllers.js`: Idempotent Piñata seed in bootstrap
...
- [x] **1.4** — `api/controllers/events.controllers.js`: Update `calculateEventPrice` to sum catalog items + Piñata dual-write
...
- [x] **1.5** — `api/controllers/events.controllers.js`: PATCH invalidation in both passes for catalog fields
...
- [x] **1.6** — `api/services/google.service.js`: Include catalog items in Google Calendar event description
...
- [x] **1.7** — `api/config/mailer.config.js`: Add HTML block for catalog items in confirmation email
