	...
	return (
		<div className="flex flex-col h-full">
			...
			<div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex justify-between items-center mb-4 shrink-0">
				<div>
					<p className="font-bold text-gray-800">Adultos Estimados</p>
					<p className="text-xs text-gray-400">Aproximado</p>
				</div>
				<div className="flex items-center gap-3 bg-gray-50 rounded-full p-1">
					<button
						...
						className="w-10 h-10 rounded-full bg-white text-gray-600 font-bold shadow-sm"
					>
						-
					</button>
					<span className="text-xl font-black w-8 text-center bg-transparent">
					...
					<button
						...
						className="w-10 h-10 rounded-full bg-gray-800 text-white font-bold shadow-sm"
					>
						+
					</button>
				</div>
							...
							<div className="flex justify-end mt-2">
								<div className="flex items-center gap-3 bg-white rounded-full p-1 shadow-sm border border-gray-100">
									<button
										...
										className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${qty > 0 ? 'bg-gray-100 text-gray-600' : 'bg-gray-50 text-gray-300'}`}
										disabled={qty === 0}
									>
										<Minus size={14} />
									</button>
									<span
										className={`text-lg font-bold w-6 text-center ${qty > 0 ? 'text-gray-800' : 'text-gray-300'}`}
									>
										{qty}
									</span>
									<button
