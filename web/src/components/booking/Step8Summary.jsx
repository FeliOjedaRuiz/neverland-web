...
const Step8Summary = ({
	...
	return (
		<div className="flex flex-col h-full">
			...
			<div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex flex-col gap-4 grow relative overflow-hidden">
				<div className="absolute top-0 left-0 w-full h-2 bg-gradient-to-r from-neverland-green via-energy-orange to-sun-yellow"></div>
				...
				{/* Detail List */}
				<div className="space-y-2 text-sm text-gray-600 flex-1 overflow-y-auto pr-2">
					<div className="flex justify-between">
						<span>
							{formData.niños.cantidad} x Menú{' '}
							{CHILDREN_MENUS.find((m) => m.id === formData.niños.menuId)?.name}
						</span>
						<span className="font-bold">
							{(
								formData.niños.cantidad *
								(prices.preciosNiños[formData.niños.menuId] || 0)
							).toFixed(2)}
							€
						</span>
					</div>
					...
					{formData.extras.taller !== 'ninguno' && (
						<div className="flex justify-between text-energy-orange">
							<span>
								Taller:{' '}
								{WORKSHOPS.find((w) => w.id === formData.extras.taller)?.name}
							</span>
							<span className="font-bold">
								{formData.niños.cantidad > 25
									? prices.preciosExtras.tallerPlus
									: prices.preciosExtras.tallerBase}
								€
							</span>
						</div>
					)}
